...
import { createBouteille, uploadPhoto } from '../api/bouteilles.api'

export default function BouteilleForm({ onAjout }) {
  const [form, setForm] = useState({
    nom: '', cepage: '', region: '', millesime: '', note: ''
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const bouteille = await createBouteille(form)
      console.log('Bouteille id :', bouteille.id)

      // 2. Si une photo est sélectionnée, l'uploader
      if (photo) {
        await uploadPhoto(bouteille.id, photo)
      }
  ...
  return (
    <form onSubmit={handleSubmit} style={styles.form}>
      <h2 style={styles.titre}>Ajouter une bouteille</h2>
      ...
      <button type="submit" disabled={loading} style={styles.btn}>
        {loading ? 'Ajout en cours...' : 'Ajouter à la cave'}
      </button>
    </form>
  )
}
